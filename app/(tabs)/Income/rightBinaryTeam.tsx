...
const RightBinaryTeam = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.get(`https://emrwalletbackend.vercel.app/users/${userId}/R?page=${page}`, {
                headers: {
    ...
    const tableHead = ['Sr No', 'UserID', 'FullName', 'Plan', 'Date'];

    const getPlanAmountStyle = (status) => {
    ...
    const renderItem = ({ item }) => (
        <View key={item.uniqueKey} style={item.id % 2 === 0 ? styles.tableRow : styles.oddRow}>
            ...
            <Text style={[styles.tableCell, getPlanAmountStyle(item.status)]}>{item.status}</Text>
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.title}>Right Binary Team</Text>
...
export default RightBinaryTeam;